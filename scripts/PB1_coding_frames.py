...
file1 = open("pb1.fasta", "w")
file2 = open("pb1-f2.fasta", "w")
...
for seq in alignment:
    ...
    # Find the first stop codon in the new reading frame starting from position 95
    for i in range(94, len(seq_str), 3):
        codon = seq_str[i:i+3].upper()
        if codon in stop_codon:
            stop2 = i
            break
    ...
    max_length_file2 = max(max_length_file2, stop2 - 94)

    # Store the sequence and stop positions for file1 and file2
    sequences.append((seq_id, seq_str[:stop1], seq_str[94:stop2]))
